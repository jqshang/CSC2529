...
class GaussianDiffusion:
    ...
    def p_mean_variance(self,
        ...
        if model_output.shape[2:] != x.shape[2:]:
            H = min(model_output.shape[2], x.shape[2])
            W = min(model_output.shape[3], x.shape[3])
            x = x[:, :, :H, :W]
        ...
        def process_xstart(x):
            ...
            if clip_denoised:
                ...
                if "mean" in model_kwargs and "std" in model_kwargs:
                    ...
                    x = (x * model_kwargs["std"][None, :, None, None] +
    ...
    def ddim_sample(
        ...
        # Make sure x and pred_xstart have the same spatial size.
        pred_xstart = out["pred_xstart"]
        if x.shape[2:] != pred_xstart.shape[2:]:
            H = min(x.shape[2], pred_xstart.shape[2])
            W = min(x.shape[3], pred_xstart.shape[3])
            x = x[:, :, :H, :W]
            pred_xstart = pred_xstart[:, :, :H, :W]
            out["pred_xstart"] = pred_xstart
            out["mean"] = out["mean"][:, :, :H, :W]
            out["variance"] = out["variance"][:, :, :H, :W]
            out["log_variance"] = out["log_variance"][:, :, :H, :W]

        # Re-derive epsilon using the (possibly cropped) tensors.
        eps = self._predict_eps_from_xstart(x, t, pred_xstart)

        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t,
                                         pred_xstart.shape)
        ...
        mean_pred = (pred_xstart * th.sqrt(alpha_bar_prev) +
                     th.sqrt(1 - alpha_bar_prev - sigma**2) * eps)
        nonzero_mask = (t != 0).float().view(
            -1, *([1] * (len(pred_xstart.shape) - 1)))  # no noise when t == 0
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if weight_logl1 > 0.0:
                logl1 = weight * logl1_loss(model_output_normalized,
                                            target_normalized)
                terms["logl1"] = logl1
