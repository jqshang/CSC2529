def mod_config(cfg):
    if cfg.model.channel_mult == "":
        if cfg.general.image_size == 512:
            ...
            raise ValueError(
                f"unsupported image size: {cfg.general.image_size}")
    else:
    ...
    cfg.model.out_channels = (cfg.model.out_channels
                              if not cfg.diffusion.learn_sigma else 2 *
                              cfg.model.out_channels)

    if "controlnet" in cfg and cfg.controlnet is not None:
        if cfg.controlnet.channel_mult == "":
            if cfg.general.image_size == 512:
                cfg.controlnet.channel_mult = (0.5, 1, 1, 2, 2, 4, 4)
            elif cfg.general.image_size == 256:
                cfg.controlnet.channel_mult = (1, 1, 2, 2, 4, 4)
            elif cfg.general.image_size == 128:
                cfg.controlnet.channel_mult = (1, 1, 2, 3, 4)
            elif cfg.general.image_size == 64:
                cfg.controlnet.channel_mult = (1, 2, 3, 4)
            else:
                raise ValueError(
                    f"unsupported image size: {cfg.general.image_size}")
        else:
            cfg.controlnet.channel_mult = tuple(
                int(ch_mult)
                for ch_mult in cfg.controlnet.channel_mult.split(","))

        if cfg.controlnet.attention_resolutions:
            cfg.controlnet.attention_resolutions = cfg.controlnet.attention_resolutions.split(
                ",")
